    ...
    <div className="overflow-hidden rounded-xl border border-white/5 bg-[#141414]">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/5 px-6 py-4">
        ...
        <div className="flex items-center gap-4 text-[11px]">
          <div className="flex items-center gap-1.5">
            <span className="flex size-4 items-center justify-center rounded-[4px] bg-[#00C853]/90">
              <CheckIcon className="size-2.5 text-[#0D0D0D]" strokeWidth={3.5} />
            </span>
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-xs">
          <thead>
            <tr className="h-28">
              <th className="sticky left-0 z-10 w-40 bg-[#141414] px-4 align-bottom pb-2 text-left font-mono text-[9px] tracking-wider text-white/40 uppercase">
                FROM ↓ TO →
              </th>
              ...
              <th className="sticky right-0 z-10 bg-[#141414] px-3 align-bottom pb-2 text-right font-mono text-[9px] tracking-wider text-white/40 uppercase">
                Out
              </th>
            </tr>
          </thead>
          <tbody>
            {crossPostMembers.map((from, i) => {
              ...
              return (
                <tr key={from.id} className="border-t border-white/5">
                  <th
                    scope="row"
                    className={`sticky left-0 z-10 bg-[#141414] px-4 ${rowPad} text-left font-mono text-[11px] font-medium text-white/85 whitespace-nowrap`}
                  >
                    {from.lastName}
                  </th>
                      ...
                      <td key={to.id} className="p-0.5">
                        {isDiag ? (
                          ...
                          <div
                            ...
                            className={`mx-auto flex ${cellSize} items-center justify-center rounded-[4px] bg-[#00C853]/85 shadow-[0_0_10px_-3px_rgba(0,200,83,0.8)]`}
                          >
                            <CheckIcon
                              className={compact ? "size-2 text-[#0D0D0D]" : "size-3 text-[#0D0D0D]"}
                              strokeWidth={3.5}
                            />
                          </div>
                        ) : (
                  ...
                  <td className={`sticky right-0 z-10 bg-[#141414] px-3 ${rowPad} text-right font-mono text-xs text-white/70 tabular-nums`}>
                    {outCount}
                  </td>
                </tr>
              );
            ...
            <tr className="border-t border-white/5 bg-[#101010]">
              <th
                scope="row"
                className="sticky left-0 z-10 bg-[#101010] px-4 py-2 text-left font-mono text-[10px] tracking-wider text-white/45 uppercase"
              >
                Incoming
              </th>
              {crossPostMembers.map((m) => {
                ...
                return (
                  <td
                    key={m.id}
                    className={`px-0 py-2 text-center font-mono text-[11px] tabular-nums ${good ? "text-[#00C853]" : "text-[#FFA000]"}`}
                  >
                    {c}
                  </td>
                );
              })}
              <td className="sticky right-0 z-10 bg-[#101010] px-3 py-2 text-right" />
            </tr>
          </tbody>
        </table>
      </div>
